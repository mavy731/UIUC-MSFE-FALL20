...
def train_and_valuation(Tree, x_train, y_train):
...
for n in RS_range:
    ...
    train_and_valuation(Tree,X_train, y_train)
...
Tree.fit(X_train, y_train)
y_test_pred=Tree.predict(X_test)
scoreh=accuracy_score(y_test,y_test_pred)
print('Holdout accurracy: %.5f' %scoreh)
